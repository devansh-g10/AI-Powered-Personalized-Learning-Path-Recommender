...
export default function CoursesPage() {
  ...
  const enrolledProgressMap = useMemo(() => {
    ...
    allCoursesCatalog.forEach((course) => {
      const stored = localStorage.getItem(`completed_topics_${course.slug}`) ||
  ...
  // Recommended list for highlight section
  const recommendedCourses = useMemo(() => {
    return allCoursesCatalog.filter((c) => c.isRecommended);
  }, []);

  return (
    <div className="flex flex-col gap-10 pb-20 w-full max-w-7xl mx-auto">
      {/* ─── 1. Hero Header Experience ────────────────────────────────────── */}
      <RevealOnScroll distance={20} duration={0.6}>
        <div className="flex flex-col gap-5 pt-2">
          <div className="flex items-center gap-2 flex-wrap">
            <Badge
              variant="secondary"
              className="font-semibold rounded-full bg-[#2b7fff]/10 text-[#2b7fff] text-xs px-3.5 py-1 gap-1.5 w-fit border border-[#2b7fff]/20"
            >
              <Sparkles className="size-3.5" />
              AI-Curated Engineering Catalog
            </Badge>

            <span className="text-xs text-zinc-500 font-medium">
              {allCoursesCatalog.length} Comprehensive Learning Programs
            </span>
          </div>

          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
            <div className="flex flex-col gap-2 max-w-3xl">
              <h1 className="font-display font-bold text-3xl sm:text-4xl md:text-5xl text-zinc-950 tracking-tight leading-[1.15]">
                Learn what moves your{" "}
                <span className="text-[#2b7fff]">career forward.</span>
              </h1>
              <p className="text-zinc-600 text-sm sm:text-base leading-relaxed">
                Explore structured engineering courses and let PathAI adapt the learning journey to your current skills, goals, and pace.
              </p>
            </div>
            ...
            <div className="relative w-full md:w-80 shrink-0">
              ...
              <input
                ...
                className="w-full h-11 pl-10 pr-4 rounded-2xl border border-zinc-200/90 bg-white text-xs sm:text-sm outline-none focus:ring-2 focus:ring-[#2b7fff]/30 focus:border-[#2b7fff] shadow-sm transition-all text-zinc-950 placeholder:text-zinc-400"
              />
              {searchQuery && (
                <button
                  type="button"
                  onClick={() => setSearchQuery("")}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-zinc-400 hover:text-zinc-700 bg-transparent border-0 cursor-pointer"
                >
                  Clear
                </button>
              )}
      ...
      <div className="flex flex-col gap-4">
        {/* Status Filter Tabs (All, Recommended, In Progress) */}
        <div className="flex items-center gap-2 overflow-x-auto pb-1 select-none border-b border-zinc-200/80">
          {[
            ...
            return (
              <button
                ...
                className={`relative px-4 py-2.5 text-xs sm:text-sm font-semibold transition-colors cursor-pointer whitespace-nowrap ${
        ...
        {/* Categories Pills */}
        <div className="flex items-center gap-2 overflow-x-auto pb-2 pt-1 select-none">
          <button
            type="button"
            ...
            className={`px-3.5 py-1.5 rounded-full text-xs font-semibold transition-all cursor-pointer whitespace-nowrap border ${
              selectedCategory === "All"
                ? "bg-[#2b7fff] text-white border-[#2b7fff] shadow-sm shadow-[#2b7fff]/25"
          ...
          {courseCategoriesList.map((cat) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-semibold transition-all cursor-pointer whitespace-nowrap border ${
                  isSelected
                    ? "bg-[#2b7fff] text-white border-[#2b7fff] shadow-sm shadow-[#2b7fff]/25"
                    : "bg-white text-zinc-700 border-zinc-200 hover:border-zinc-300 hover:bg-zinc-50"
                }`}
      ...
      {/* ─── 3. Recommended For You Highlight Section ──────────────────────── */}
      {activeTab === "all" && selectedCategory === "All" && !searchQuery && (
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-zinc-500 font-medium">
              Based on your active roadmap & verified competencies
            </span>
          </div>
...
function CourseCard({ course, progress, onSelect }: CourseCardProps) {
  ...
  return (
    <Card
      onClick={onSelect}
      className="group glass-card backdrop-blur-xl bg-white/85 border-zinc-200/80 hover:border-[#2b7fff]/40 shadow-sm hover:shadow-xl hover:shadow-[#2b7fff]/8 rounded-3xl p-6 flex flex-col justify-between gap-5 transition-all duration-300 cursor-pointer hover:-translate-y-1"
    >
      <CardHeader className="p-0 gap-3">
        ...
        <div className="flex items-center justify-between gap-2">
          <Badge
            variant="outline"
            className="text-[11px] font-semibold text-zinc-600 border-zinc-200/80 bg-zinc-50/80 flex items-center gap-1"
          >
          ...
          {course.isRecommended && (
            <Badge className="bg-[#2b7fff]/10 text-[#2b7fff] border-[#2b7fff]/20 text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
      ...
      <CardContent className="p-0 flex flex-col gap-4">
        {/* Prerequisites & Beginner-First Note */}
        <div className="flex items-center gap-2 flex-wrap text-[11px] text-zinc-500">
          ...
          <Badge
            variant="secondary"
            className={`text-[10px] px-2 py-0 font-bold rounded-md ${
        ...
        {/* Prerequisites Check Preview */}
        <div className="p-2.5 rounded-xl bg-zinc-50 border border-zinc-100 text-[11px] text-zinc-600 flex items-center justify-between">
      ...
      <CardFooter className="p-0 pt-2 flex items-center justify-between border-t border-zinc-100/90">
        ...
        <div className="flex items-center gap-1 text-xs font-bold text-[#2b7fff] group-hover:translate-x-1 transition-transform">
